...
const Random = {
  ...
  randomNumber(max) {
    ...
    var rand = Random.getNextRandomValue();
    while (rand >= 256 - (256 % max)) {
      rand = Random.getNextRandomValue();
    }
    return rand % max;
  },
  randomPassword(options, pool) {
    var password = '',
      optionsLength = options.length,
      poolLength = pool.length;
    for (var i = 0; i < optionsLength; i++) {
      password += pool[Random.randomNumber(poolLength)];
    }
    if (options.strict) {
      ...
      if (!fitsRules) return Random.randomPassword(options, pool);
    }
    return password;
  },
  getRandomPassword(options = null) {
    // Set defaults.
    options = options || {};
    ...
    if (!Object.prototype.hasOwnProperty.call(options, 'strict')) options.strict = true;
    ...
    var password = Random.randomPassword(options, pool);
